...
```
if the typeof callback is function, the sign will use that for the callback instead of return the value,
callback params will be callback(err, token)

## Verify method
```nodejs
jwtExpress.verify(token, options = {}, callback = null, onlyPayload = true) {
```
if the typeof callback is function, the sign will use callback instead of return the value,
callback params will be callback(err, token)
